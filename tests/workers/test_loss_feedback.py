...
class TestTriggerOnNegativeRollingPnl:
    def test_negative_rolling_pnl_triggers(self):
        # 5 trades summing to -5 (window=5 in default cfg)
        trades = _make_trades([-10, 3, -2, 1, -2])
        # consecutive losses = 1 (below trigger=3), but rolling P&L < 0
        result, mock_redis = _patched_run(trades)

        assert result["triggered"] is True
        assert result["adjusted"] is True
        assert result["rolling_net_pnl"] < 0
