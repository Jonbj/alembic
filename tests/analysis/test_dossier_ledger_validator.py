...
def test_validate_panels_rigetta_doppio_primary_per_stesso_ticker_day():
    ...
    occ = [
        ...
        _occ_ok(
            causal_event_id="trade:726",
            primary_finding="F-002",
            actual_usd=12.5,
            segment="trade",
        ),
    ]
...
def test_validate_findings_rigetta_occorrenza_prima_di_primo_avvistamento():
    ...
    assert not res["ok"]
    assert any(
        "avvistamento" in e.lower() or "primo" in e.lower() for e in res["errors"]
    )
...
def test_validate_panels_rigetta_dossier_hash_non_corrispondente():
    ...
    res = lv.validate_panels(
        ...
        dossier_hashes={"2026-08-04": "actual"},
        window=WINDOW,
    )
    assert not res["ok"]
...
def test_validate_panels_segnala_ticker_day_mancante_nel_pannello():
    # il pannello ticker-day copre AAPL ma il dossier ha due movers: MSFT manca.
    ticker_day = [
        {
    ...
    res = lv.validate_panels(
        {"occurrences": [_occ_ok()], "definitions": [], "ticker_day": ticker_day},
        dossier_hashes={"2026-08-04": "h"},
        window=WINDOW,
