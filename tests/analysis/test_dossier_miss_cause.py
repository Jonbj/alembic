...
def test_sopra_gate_non_e_un_miss_classificabile_da_questa_tassonomia():
    ...
    comunque come BELOW_GATE per non lasciare una chiave vuota (raro, ma
    difendiamo il campo)."""
    out = classify_miss_candidates([_cand("X", news_count=1, segnali=[
        {"ora": "15:00", "score": 0.45, "fallback": False}
    ])])
    # Non e' nessuna delle 5 cause riconosciute; il classificatore segnala 'NON_CLASSIFICATO'.
...
def test_regressione_4_giorni_riproduce_la_tabella_della_issue():
    ...
    atteso = [
        # 2026-08-03: NO_NEWS=3 (BA, HOOD, SAP), BELOW_GATE=4 (RDDT, SNOW, BABA, TSLA),
        # THIN_NEUTRAL=2 (AZN, SPCX) -- NB SPCX ha 0.250 BELOW_GATE, rivedi
        # ATTESA ORIGINALE DELLA ISSUE: NO_NEWS=3, con_news=6
        ("2026-08-03", candidati_0803, {"NO_NEWS": 3, "BELOW_GATE": 4, "THIN_NEUTRAL": 2}),
        # 2026-08-04: NO_NEWS=6, BELOW_GATE=2 (SPCX=0.450 -> NON_CLASSIFICATO, AVGO=0.276),
        # THIN_NEUTRAL=1 (IBM)
