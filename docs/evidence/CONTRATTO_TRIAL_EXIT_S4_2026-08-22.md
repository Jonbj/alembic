...
| 2026-09-09 | **Registrata l'inflazione effettivamente applicata al calcolo di `N_cluster`: nessuna** (`dependence_inflation_applied: 1.0`, `missingness_rate_applied: 0.0`, i default di `scripts/check_s4_trial_milestones.py`). La riga 15 di §8.4 impone l'inflazione «per dipendenza e missingness» ma non ne fissa i valori; senza questa annotazione il registro avrebbe attestato una derivazione «con la formula pre-registrata» tacendo che una sua componente obbligatoria era stata lasciata neutra. **5816 va quindi letto come limite inferiore di `N_cluster`, non come stima finale**; i due fattori vanno fissati con giustificazione alla ri-stima blinded al 50%, l'unica concessa. | **Non materiale** — annota il valore usato, non cambia `N_cluster` né la formula. | Completezza del registro: una derivazione parziale dichiarata come completa non è verificabile a posteriori. | questo commit |
