...
| R-07 | **Controlled paper trading not started** — no live-market data has flowed through the production stack; all validation is against historical data and paper-simulated order flow | HIGH | N/A (known gap) | Open — preflight runbook complete; dry-run executed (3 blockers found + fixed: BUG-5 admin.py bytes/str, BUG-6 cockpit dual-path); PO sign-off + strategy approval governance remaining | Maintainer | Runbook: `docs/CONTROLLED_PAPER_PREFLIGHT_RUNBOOK_2026-06-21.md`; engineering blockers BUG-5/BUG-6 closed 2026-06-22 |
...
| R-12 | **S7 wiring risk** — S7 (PEAD) code is implemented and tested but intentionally NOT wired into PortfolioOrchestrator; risk if future session accidentally wires it without completing research phase | MEDIUM | Low | Mitigated (P0-13) | Maintainer | `allocation_pct=0.0` enforced; state is `research`; no promotion gate has passed |
