...
| `MobileMonitorSnapshot` | `src/workers/mobile_monitor_task.py` | Produces one coherent atomic Redis read model per minute and persists NAV history every five minutes when broker NAV is available |
