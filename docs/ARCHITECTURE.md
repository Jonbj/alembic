...
| ~~Strategies API placeholder~~ | **RESOLVED by removal (2026-09-02, PR #471)**: the read endpoints (`GET /api/strategies`, `/{id}`, `/backtest`, `/gates`, `/sensitivity`) and the frontend page were deleted rather than fixed. The gap was real and wider than this row said: the sensitivity grid was generated in code from a closed-form gaussian, the equity curve read a gitignored `reports/` tree (empty in the deploy worktree), the gate panel showed invented thresholds *stricter* than the real ones in `reports/s1_backtest/gate_report.json` (~0.0) with two gates reporting PASS below their own displayed threshold, and `total_trades` claimed 1247 for S1 against 103 real rows. Authorization state — the only correct part — moved to `GET /portfolio/status`. The three promotion-gate POST endpoints survive. | Done |
