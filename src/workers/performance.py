...
def _load_loss_feedback_config() -> dict:
    ...
    defaults = {
        ...
        # #191: tetto del ratchet. False congela SOLO l'innalzamento
        # dell'entry threshold (che resta ancorata al baseline); regime_scale,
        # stato e S1 restano vivi. True è il default storico (retrocompatibile):
        # il flip è una decisione di taratura dell'operatore, non del codice.
...
def run_loss_feedback_check() -> dict:
    ...
    for strategy in sorted(fb._history.keys()):
        ...
        if outcome.triggered and cooldown_ok and stale_evidence:
            ...
            ratchet_enabled = cfg.get("threshold_ratchet_enabled", True)
            if ratchet_enabled:
