...
app.conf.beat_schedule = {
    ...
    # Intraday fill reconciliation every 15 min during market hours.
    # Fires 5 min after each portfolio cycle (:07/:22/:37/:52) to allow
    # Alpaca fills to settle before fetching. Populates exit_price + net_pnl
    # so the daily P&L tab shows intraday closed trades without waiting for EOD.
    "reconcile-fills-intraday": {
        "task": "src.workers.performance.run_reconcile_fills_intraday",
        "schedule": crontab(minute="12,27,42,57", hour="14-21", day_of_week="1-5"),
    },
